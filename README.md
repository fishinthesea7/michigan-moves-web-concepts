...
Directory organization logos live in `docs/assets/images/member-logos/`. Upload each approved logo to the WordPress Media Library, then replace its relative `organizationLogoUrl` in `directory-data.js` with the Media Library file's full HTTPS URL. Load `directory-data.js` before `directory.js`; the cards are created at runtime, so separate Elementor Image widgets are not needed. Preserve the `.mmc-org-logo` rules when copying the namespaced CSS: the circular wrapper clips the asset, while `object-fit: contain` and internal padding keep wide, tall, and square logos inside the same boundary. For the cleanest manual result, use a transparent PNG or SVG with close-cropped artwork and preview it at 80 by 80 pixels. When one logo needs more or less breathing room, add `organizationLogoPadding: 4` to that record and adjust the number from 0 through 20 instead of changing the shared circle dimensions. Records without an approved logo intentionally show organization initials.
